/*@desc Trie Articles 
 * The Program reads company names from input file : company.dat 
 * A company might have multiple names associated with it, The first name is the primary name, and the remainder are synonyms
 * The program saves these names in a trie structure for efficient comparison and calculation of company instance occurrences  
 * The company occurrences are counted as the program reads a news article from standard input and keeps reading until 
 * a line in the article that consists entirely of a period symbol (.).
 * 
 * The program always display the primary name of the company identified. It also displays the "relevance" of the 
 * company name hit. Relevance is defined as frequency of the company name appearing in the article divided by the  
 * number of words in the article. While calculating the relevance of the company name, words like (a, an, the, and, or, but)
 * are ignored. 
 * Company names are normalized for the search. Punctuation and other symbols do not impact the search, however  
